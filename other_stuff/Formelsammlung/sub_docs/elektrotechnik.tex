...
\includestandalone{figures/c_ui_diagram.tikz}
%\subfile{figures/c_ui_diagram.tikz.tex}
...
\begin{sectionbox}
	...
	\subsection{Scheitelfaktor}
	Verhältnis Scheitelwert zu Effektivwert einer Wechselgrösse, auch: Crestfaktor
	\begin{emphbox}
		Scheitelfaktor: $k_s = \frac{\abs{X}_{max}}{X_{eff}}$
	\end{emphbox}

	Verhältnis Spitzenleistung zur mittleren Leistung eines Signals, auch: PAPR (Peak-to-Average Power Ratio)

	\begin{emphbox}
		PAPR: = $ {k_s}^2 = \frac{\abs{X}^2_{max}}{X^2_{eff}} $ 
	\end{emphbox}
...
\begin{sectionbox}
		...
		\begin{emphbox}
			\begin{align*}
				\text{Polarform}: {\underline {Z}} &= |{\underline {Z}}|\cdot e^{\mathrm {j} \varphi } \\
				\text{kart. Form}: \underline {Z} &= R + \mathrm {j} X \\
				\text{Wirkwiderstand (Resistanz)}: R &= \Re(\underline{Z}) = Z \cos \varphi \\
				\text{Blindwiderstand (Reaktanz)}: X &= \Im(\underline{Z}) = Z \sin \varphi \\
				\text{Scheinwiderstand}: Z &= |{\underline {Z}}| = \frac{\hat{u}}{\hat{\i}} = \frac{U_{eff}}{I_{eff}}
			\end{align*}
		\end{emphbox}
%	{\displaystyle {\underline {Z}}} 	  	Impedanz (komplexe Impedanz)
%	
		\subsection{Admittanz}
	
		Komplexer Leitwert, Kehrwert der Impedanz \underline {Z}
		\begin{emphbox}
			\begin{align*}
				\text{Admittanz}: {\underline {Y}} &= \underline {Z}^{-1} \\
				\text{kart. Form}: {\underline {Y}} &= G + \mathrm {j} B \\
				\text{Wirkleitwert (Konduktanz)}: G &= \Re(\underline {Y}) \\
				\text{Blindleitwert (Suszeptanz)}: B &= \Im(\underline {Y}) \\
				\text{Scheinleitwert}: Y &= |{\underline {Y}}| = \sqrt{G^2 + B^2}
			\end{align*}
		\end{emphbox}
...
Logarithmische Masseinheiten:
	...
	dBm etc.
Dekade +10 bzw. /10 
Oktave (bei Frequenzverhältnissen) = *2 bzw. /2
	Logarithmische Verhältnisse:

	\begin{tabular}{c|c|c|c|c|c}
		Q & $ P_1/P_2 $ & $ F_1/F_2 $ & Q & $ P_1/P_2 $ & $ F_1/F_2 $\\
		40 dB & 10000 & 100 & -40 dB & 0.0001 & 0.01 \\
		20 dB & 100 & 10 & -20 dB& 0.01 & 0.1 \\
		10 dB & 10 & $\approx$3.16 & -10 dB& 0.1 & $\approx$0.32 \\
		6 dB & $\approx$4 & $\approx$2 & -6 dB &$\approx$0.25 & $\approx$0.5\\
		3 dB & $\approx$2 & $\approx$1.41 & -3 dB & $\approx$0.5&$\approx$0.71 \\
		1 dB & $\approx$1.26 & $\approx$1.12 & -1 dB & $\approx$0.79&$\approx$0.89\\
		0 dB & 1 & 1 & & & \\
	\end{tabular}
...
Kontinuierliche LTI-Systeme:
	\begin{emphbox}
		$ H(s) = \frac{Y(s)}{X(s)} = \frac{\mathcal{L}\{y(t)\}}{\mathcal{L}\{x(t)\}} $ bzw. $ Y(s) = H(s) \cdot X(s) $
	\end{emphbox}
\end{sectionbox}
\begin{sectionbox}
	Zeitdiskrete LTI-Systeme:
	\begin{emphbox}
		$ H(z) = \frac{Y(z)}{X(z)} = \frac{\mathcal{Z}\{y[k]\}}{\mathcal{Z}\{x[k]\}} $
	\end{emphbox}
\end{sectionbox}


Laplace: $ s = \sigma + j \omega $ (transient and steady state)

Fourier: $ s = j \omega $ mit $\sigma = 0$ (steady state only)
...
\begin{tikzpicture}
	\node (OL) [draw=black] {Signale im kont. Zeitbereich};
	\node (OR) [draw=black,right=3cm of OL,align=center] {Spektralbereich\\ Laplace-Bildbereich};
	\node (UL) [draw=black,below=of OL] {Signale im diskr. Zeitbereich};
	\node (UR) [draw=black,right=3cm of UL,align=center] {Spektralbereich\\ z-Bildbereich};
	\draw (OL) edge[->] node[above,align=center] {Laplace-Transformation\\Abtastung} (OR);
	\draw (OL) edge[->] node[right,align=center] {Diskretisierung\\Abtastung} (UL);
	\draw (UL) edge[->] node[above,align=center] {z-Transformation} (UR);
\end{tikzpicture}
